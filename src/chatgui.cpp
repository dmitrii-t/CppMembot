...
ChatBotFrame::ChatBotFrame(const wxString& title)
    ...
    int idTextXtrl = 1;
    _userTextCtrl =
        new wxTextCtrl(ctrlPanel, idTextXtrl, "", wxDefaultPosition, wxSize(width, 50),
...
void ChatBotPanelDialog::AddDialogItem(wxString text, bool isFromUser) {
    ...
    _dialogSizer->Add(item, 0,
                      wxALL | (isFromUser == true ? wxALIGN_LEFT : wxALIGN_RIGHT), 8);
    _dialogSizer->Layout();
...
ChatBotPanelDialogItem::ChatBotPanelDialogItem(wxPanel* parent, wxString text,
    ...
    wxBitmap* bitmap = isFromUser == true ? NULL
    ...
    _chatBotTxt = new wxStaticText(this, wxID_ANY, text, wxPoint(-1, -1),
                                   wxSize(150, -1), wxALIGN_CENTRE | wxBORDER_NONE);
